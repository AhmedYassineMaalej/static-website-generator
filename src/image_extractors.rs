...
impl Extractor<Vec<ArticleImage>> for ImageExtractor {
    fn consume(self) -> Vec<ArticleImage> {
        self.images
    }
}

impl MarkdownVisitor<()> for ImageExtractor {
    ...
    fn visit_image(&mut self, image: &markdown::mdast::Image) {
        ...
        let image = if url.starts_with("http") {
            ArticleImage::Online(url.clone())
        } else {
            ArticleImage::Local(PathBuf::from(url))
        };
